...
class TestEndToEndPipeline(unittest.TestCase):
        ...
        try:
            ...
            try:
                X, Y, orient, dzeta = measure.orientation_map(
                    ims, logfile, tmin=0, tmax=1, xstep=16, ystep=16, patchw=16
                )
    ...
    def test_motion_limits_and_angles_pipeline(self):
        ...
        # Create logfile
        logfile = {"detector": {"frames": np.zeros((nt, 3))}}
    ...
    def test_tiff_export_pipeline(self):
        ...
        # Export to TIFFs
        io.save_as_tiffs(output_folder, data, normalisation=exposure.mean_std, tmin=0, tmax=5, tstep=1)
...
class TestHardcodedIssues(unittest.TestCase):
    ...
    def test_normalise_rotation_works_correctly(self):
        ...
        bg_logfile = {"detector": {"frames": np.column_stack([np.arange(nt), np.arange(nt), np.linspace(0, 360, nt)])}}

        fg_logfile = {"detector": {"frames": np.column_stack([np.arange(nt), np.arange(nt), np.linspace(0, 360, nt)])}}
